...
module.exports = welcome = async (Marcel, anu) => {
		...
		try {
                ...
                pp_user = await Marcel.getProfilePicture(mem)
                ...
                pp_grup = await Marcel.getProfilePicture(anu.jid)
            ...
            if (anu.action == 'add' && mem.includes(Marcel.user.jid)) {

            Marcel.sendMessage(anu.jid, 'Halo! Terima Kasih sudah Mengundangku, Jika ingin Menggunakan Bot Ketik !menu', 'conversation')
             ...
             if (!welkom.includes(anu.jid)) return
                ...
                mdata = await Marcel.groupMetadata(anu.jid)
                ...
                let v = Marcel.contacts[num] || { notify: num.replace(/@.+/, '') }
        ...
        buttons = [
          ...
          { buttonId: `!grupinfo`, buttonText: { displayText: "INFOGRUP" }, type: 1 },
      ...
      if (anu.action == "remove" && !mem.includes(Marcel.user.jid)) {
        ...
        let w = Marcel.contacts[num] || { notify: num.replace(/@.+/, "") };
